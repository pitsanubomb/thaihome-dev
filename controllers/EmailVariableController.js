                        ...
                        if(!err){
                            Property.findOne({_id:booking.property}, function(err, property){
                                if(!err){
                                    ...
                                    var PERIOD = x.join(', ');
                                    user = user[0];
                                    function GenerateVariables(func, condition){
                                        var funcFromData = func;
                                        console.log(funcFromData);
                                        if(eval(condition)){
                                    ...
                                    for(var i = 0; i < variables.length; i++){
                                        convertedVariables[variables[i].variable] = GenerateVariables(variables[i].func, variables[i].func);
                                    }
